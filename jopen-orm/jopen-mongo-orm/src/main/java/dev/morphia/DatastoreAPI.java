 ...
 * @since 2019/10/17
 */
public class DatastoreAPI implements AdvancedDatastore {
    ...
    public DatastoreAPI(final Morphia morphia, final MongoClient mongoClient, final String dbName) {
        this(morphia, morphia.getMapper(), mongoClient, dbName);
    }

    @Deprecated
    public DatastoreAPI(final Morphia morphia, final Mapper mapper, final MongoClient mongoClient, final String dbName) {
        this(morphia, mapper, mongoClient, mongoClient.getDatabase(dbName));
    }


    private DatastoreAPI(final Morphia morphia, final Mapper mapper, final MongoClient mongoClient, final MongoDatabase database) {
    ...
    public <T> MapreduceResults<T> mapReduce(final MapReduceOptions<T> options) {
        ...
        if (MapReduceCommand.OutputType.INLINE.equals(options.getOutputType())) {
    ...
    public <T> MapreduceResults<T> mapReduce(final MapreduceType type, final Query query, final Class<T> outputType,
        ...
        final MapReduceCommand.OutputType outType = type.toOutputType();
...
*/
    ...
    private <T> List<Key<T>> postSaveOperations(final Iterable<T> entities, final Map<Object, DBObject> involvedObjects,
        ...
        for (Map.Entry<Object, DBObject> entry : involvedObjects.entrySet()) {
